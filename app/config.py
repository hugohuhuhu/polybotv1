...
class Settings(BaseSettings):
    ...
    book_fetch_timeout_sec: float = Field(default=10.0, alias="BOOK_FETCH_TIMEOUT_SEC")
    book_fetch_retries: int = Field(default=3, alias="BOOK_FETCH_RETRIES")
    ...
    near_close_crypto_updown_fast_scan_sec: float = Field(
        default=2.0,
        alias="NEAR_CLOSE_CRYPTO_UPDOWN_FAST_SCAN_SEC",
    )
    ...
    near_close_crypto_updown_wrong_resolution_cooldown_buckets: int = Field(
        default=1,
        alias="NEAR_CLOSE_CRYPTO_UPDOWN_WRONG_RESOLUTION_COOLDOWN_BUCKETS",
    )
    ...
    auto_redeem_wrap_allowance_usdce: float = Field(default=10000.0, alias="AUTO_REDEEM_WRAP_ALLOWANCE_USDCE")
