...
- `NEAR_CLOSE_CRYPTO_UPDOWN_TAKER_FALLBACK_ENABLED=true` in the local launch scripts allows a strict FAK taker fallback only for crypto Up/Down Gate 8 entries from 30-45 seconds to close, with best ask `<=0.90`, spread `<=0.02`, ask depth at least the order size, and start distance at least `2x` the active threshold. Outside those limits the normal post-only maker path remains in force.
