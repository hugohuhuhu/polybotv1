...
- crypto Up/Down start-distance requirements are multiplied by `NEAR_CLOSE_WEEKEND_START_DISTANCE_MULTIPLIER` (`0.21` in the launch scripts)

This is intentionally the only weekend criteria relaxation. Spread remains tightened by the weekend multiplier, depth stays unchanged, and the bid skip still prevents chasing `best_bid >= 0.96`. The lowest dynamic start-distance rung remains above the cancel line (`0.00085 * 0.21 = 0.0001785 > 0.00012`) so a newly eligible order does not immediately trip the start-distance live guard.
